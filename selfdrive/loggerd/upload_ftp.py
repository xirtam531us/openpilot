...
def get_arg_parser():
  parser = argparse.ArgumentParser(description='Upload to ftp server', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  parser.add_argument("file_path", nargs='?', default="/data/test.txt", help="Path to the file")
  return parser
...
def upload_to_ftp(dongle_id, key, file_path):
  ...
  ftp = ftplib.FTP('mybooklive')
  ftp.login("openpilot", "asdf")
  with open(file_path, 'rb') as f:
    remote_dir = os.path.join('/openpilot', dongle_id, os.path.dirname(key))
